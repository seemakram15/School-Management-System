...
export default function GradeNewPage() {
  ...
  const [form, setForm] = useState({ academic_year_id: "", name: "", percent_from: "", percent_to: "", grade_point: "", pass_mark: "" });

  useEffect(() => {
    fetch("/api/academic/years").then(r => r.json()).then((yrs: AcYear[]) => {
      setAcademicYears(yrs);
      const running = yrs.find(y => y.is_running);
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Add New Grade</p>
      </div>
      <div className="bg-card rounded-xl border border-border shadow-sm p-6 max-w-xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button type="submit" disabled={loading} className="ml-auto">{loading ? "Saving..." : "Add Grade"}</Button>
          </div>
        </form>
      </div>
    </div>
