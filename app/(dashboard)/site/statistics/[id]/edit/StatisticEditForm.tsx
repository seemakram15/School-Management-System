...
export default function StatisticEditForm({ statistic }: { statistic: IStatistic }) {
  ...
  const [form, setForm] = useState({
    ...
    icon: statistic.icon ?? "",
    order: String(statistic.order ?? 0),
    status: String(statistic.status ?? 1),
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/site/statistics/${statistic.id}`, {
        method: "PATCH",
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button type="submit" disabled={loading} className="ml-auto">{loading ? "Saving..." : "Update Statistic"}</Button>
          </div>
        </form>
      </div>
    </div>
