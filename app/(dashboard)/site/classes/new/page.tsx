...
export default function ClassProfileNewPage() {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [classes, setClasses] = useState<IClassOption[]>([]);
  const [form, setForm] = useState({ class_id: "", description: "", image: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/site/classes", {
        method: "POST",
      ...
      toast.success("Class profile created");
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button type="submit" disabled={loading} className="ml-auto">{loading ? "Saving..." : "Add Class Profile"}</Button>
          </div>
        </form>
      </div>
    </div>
