...
interface IClass {
  ...
  have_selective_subject: boolean | null;
  max_selective_subject: number | null;
  have_elective_subject: boolean | null;
  status: number | null;
}

export default function ClassEditForm({ cls }: { cls: IClass }) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [form, setForm] = useState({
    name: cls.name,
    numeric_value: String(cls.numeric_value ?? ""),
    have_selective_subject: !!cls.have_selective_subject,
    max_selective_subject: String(cls.max_selective_subject ?? "0"),
    have_elective_subject: !!cls.have_elective_subject,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/academic/classes/${cls.id}`, {
        ...
        body: JSON.stringify({ ...form, numeric_value: parseInt(form.numeric_value) || 0, max_selective_subject: parseInt(form.max_selective_subject) || 0 }),
      });
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Edit Class</p>
      </div>
      <div className="bg-card rounded-xl border border-border shadow-sm p-6 max-w-xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button type="submit" disabled={loading} className="ml-auto">{loading ? "Saving..." : "Update Class"}</Button>
          </div>
        </form>
      </div>
    </div>
