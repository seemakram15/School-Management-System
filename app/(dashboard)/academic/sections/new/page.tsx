...
export default function SectionNewPage() {
  ...
  const [form, setForm] = useState({ name: "", class_id: "", capacity: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/academic/sections", {
        ...
        body: JSON.stringify({ ...form, class_id: parseInt(form.class_id), capacity: parseInt(form.capacity) || null, status: 1 }),
      });
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm p-6 max-w-xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button type="submit" disabled={loading} className="ml-auto">{loading ? "Saving..." : "Add Section"}</Button>
          </div>
        </form>
      </div>
    </div>
