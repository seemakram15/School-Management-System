...
// Postgres rejects "" for typed columns (date, int, etc). Forms send "" for
// untouched optional fields, so normalize those to null before any insert/update.
export function nullifyEmpty<T extends Record<string, unknown>>(body: T): T {
  const result = { ...body };
  for (const key in result) {
    if (result[key] === "") result[key] = null as never;
  }
  return result;
}
